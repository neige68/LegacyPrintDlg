...
- OWLNext 7.0.19 以上：環境変数 `OWLROOT` にパスを設定
...
コードはほぼ `src/main.cpp` 1 ファイルに集約されている。

**主要クラス（OWLNext フレームワーク）:**
- `TMyApp`（`owl::TApplication` 派生）: アプリケーションクラス、`InitMainWindow()` でメインウィンドウを生成
- `TMyFrameWindow`（`owl::TFrameWindow` 派生）: 最大化ボタン・サイズ変更枠を無効化したフレームウィンドウ
- `TMyClientDialog`（`owl::TDialog` 派生）: メインダイアログ。起動時にレジストリを読んでラジオボタンを設定、OK 時に書き込み
- `TAboutDialog`（`owl::TDialog` 派生）: 「アプリについて」ダイアログ。バージョン表示と GitHub リンク

**主要関数:**
- `ReadTheRegKey()`: レジストリから `PreferLegacyPrintDialog` を読み取る
...
- プリコンパイルヘッダ: `pch.h` / `pch.cpp`（OWLNext の `_OWLPCH` を使用）
